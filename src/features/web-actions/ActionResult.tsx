  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-sm border border-[var(--color-surface-container-high)] bg-[var(--color-surface)]">
      ...
      <div className="px-4 pt-4 pb-3 flex items-start gap-3">
        <div className="flex-shrink-0 w-7 h-7 rounded-full bg-[var(--color-primary)] flex items-center justify-center mt-0.5">
      ...
      <div className="mx-4 mb-4">
        {succeeded && (
          <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-[var(--color-tertiary-container)] border border-[var(--color-tertiary)]/20">
            <svg viewBox="0 0 24 24" fill="#188038" className="w-4 h-4 flex-shrink-0 mt-0.5">
              <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" />
            </svg>
            <p className="text-[var(--color-tertiary)] text-sm leading-snug">{action.result}</p>
          </div>
        )}
        {failed && (
          <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-[var(--color-error-container)] border border-[#D93025]/20">
            <svg viewBox="0 0 24 24" fill="#D93025" className="w-4 h-4 flex-shrink-0 mt-0.5">
              <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" />
            </svg>
            <p className="text-[var(--color-error)] text-sm leading-snug">{action.error}</p>
          </div>
        )}
        {!hasToolCall && !failed && (
          <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-[var(--color-warning-container)] border border-[#F9AB00]/20">
            <svg viewBox="0 0 24 24" fill="#B06000" className="w-4 h-4 flex-shrink-0 mt-0.5">
              <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" />
            </svg>
            <p className="text-[var(--color-warning)] text-sm leading-snug">
